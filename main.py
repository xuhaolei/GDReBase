    ...
    for journal in journals:
        ...
        for i in range(1, 20000):
            if journalCount == point[0] and i < point[1]:
                continue
            print(journalCount, journal[0], i)
            url = """https://pubmed.ncbi.nlm.nih.gov/?term=("%s"[Journal]) AND (("%s/%s/%s"[Date - Publication] : 
            "%s/%s/%s"[Date - Publication]))&sort=pubdate&filter=simsearch1.fha&page=%d""" % (
                '+'.join(journal[0].replace('&', '').split(' ')), journal[1][0], journal[1][1], journal[1][2],
                journal[2][0], journal[2][1], journal[2][2], i)
            print(url)
            driver.get(url)
            ...
            for article in articles:
                try:
                    ...
                    articleDic['date'] = transformDate(day=articleDic['date'][2], month=articleDic['date'][1],
                                                       year=articleDic['date'][0])
                    articleDic["institution"] = re.search(r"^(.*?)\.", texts[4]).group(1)
                    articleDic["pubmedUrl"] = article.find_elements_by_css_selector("a.docsum-title")[0].get_attribute(
                        "href")
                    # print(articleDic)
            ...
            for article in articleList:
                ...
                article['urlDic'] = {item.get_text().strip().replace(' ', ''): item.a['href'] for item in
                                     article['urlDic']}
                abstract = ''
                try:
                ...
                for item in prioritySources:
                    ...
                    # 此处直接转
                    article['contents'] = eval(
                        "%s.%s(url='%s',driver=driver).get_text()" % (str(item), str(item), str(article['url'])))
                    a = Article(article)
                    # print(article)
